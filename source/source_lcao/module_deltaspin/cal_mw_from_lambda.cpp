...
void spinconstrain::SpinConstrain<std::complex<double>>::cal_mw_from_lambda(int i_step, const ModuleBase::Vector3<double>* delta_lambda)
{
    ...
    {
        this->zero_Mi();
        int size_becp = 0, nk = 0, nkb = 0, nbands = 0, npol = 0;
        ...
        if (PARAM.inp.device == "cpu")
        {
            ...
            for (int ik = 0; ik < nk; ++ik)
            {
                ...
                hsolver::DiagoIterAssist<std::complex<double>>::diag_responce(h_tmp.data(), s_tmp.data(), nbands,
                    becp_k, &becp_tmp[ik * size_becp], nkb * npol, &this->pelec->ekb(ik, 0));
            }
        }
#if ((defined __CUDA) || (defined __ROCM))
        else
        {
            ...
            for (int ik = 0; ik < nk; ++ik)
            {
                ...
                hsolver::DiagoIterAssist<std::complex<double>, base_device::DEVICE_GPU>::diag_responce(h_tmp, s_tmp, nbands,
                    becp_k, becp_pointer, nkb * npol, &this->pelec->ekb(ik, 0));
...
#endif
    {
        ...
        if (PARAM.inp.device == "cpu")
        {
            ...
            for (int ik = 0; ik < nk; ++ik)
            {
                ...
                hsolver::DiagoIterAssist<std::complex<double>>::diag_subspace_psi(h_tmp.data(), s_tmp.data(), nbands,
                    psi_t[0], &this->pelec->ekb(ik, 0));
            }
            ...
            if(pw_solve)
            {
                hsolver::HSolverPW<std::complex<double>, base_device::DEVICE_CPU> hsolver_pw_obj(this->pw_wfc_,
                    PARAM.inp.calculation, PARAM.inp.basis_type, PARAM.inp.ks_solver, false,
                    PARAM.globalv.use_uspp, PARAM.inp.nspin,
                ...
                hsolver_pw_obj.solve(hamilt_t, psi_t[0], this->pelec, this->pelec->ekb.c,
                    GlobalV::RANK_IN_POOL, PARAM.globalv.nproc_in_pool, false, this->tpiba, this->get_nat());
            }
            else
            {
                this->pelec->psiToRho(*psi_t);
        ...
        else
        {
            ...
            for (int ik = 0; ik < nk; ++ik)
            {
                ...
                hsolver::DiagoIterAssist<std::complex<double>, base_device::DEVICE_GPU>::diag_subspace_psi(h_tmp, s_tmp, nbands,
                    psi_t[0], &this->pelec->ekb(ik, 0));
            }
