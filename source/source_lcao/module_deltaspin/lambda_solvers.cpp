    ...
    for (int i_step = -1; i_step < nsc; i_step++)
    {
        ...
        else
        {
            ...
            if (i_step >= nsc_min && grad_ok)
            {
                ...
                sc_.set_lambda(cur_lambda);
                sc_.update_psi_charge(dnu_last_step.data(), true);
...
LambdaSolverResult ChiGuidedLambdaSolver::run(int outer_step)
{
    ...
    for (int inner = 0; inner < max_inner_iter; inner++)
    {
        ...
        const double lambda_max = sc_.get_sccut();
        for (int iat = 0; iat < nat; iat++)
        {
            if (sc_.get_constrain()[iat].z == 0) { continue; }
...
// ===================================================================
// Solver 3: Subspace Diagonalization
//
// Uses LCAO subspace diagonalization for fast lambda optimization.
// Phase 1 (i_step=-1): full diag to build subspace cache
...
SubspaceLambdaSolver::SubspaceLambdaSolver(
    SpinConstrain<std::complex<double>>& sc,
    hsolver::SubspaceMode mode,
    hsolver::SubspacePrecision precision)
    : sc_(sc), subspace_solver_(new hsolver::HSolverLCAOSubspace(
          sc.ParaV, PARAM.inp.ks_solver, mode, precision))
{
}

void SubspaceLambdaSolver::set_mode(hsolver::SubspaceMode mode)
{
    subspace_solver_->set_mode(mode);
}

void SubspaceLambdaSolver::set_precision(hsolver::SubspacePrecision prec)
{
    subspace_solver_->set_precision(prec);
}

void SubspaceLambdaSolver::set_persistent(bool persistent)
{
    subspace_solver_->set_persistent(persistent);
}

bool SubspaceLambdaSolver::has_subspace() const
{
    return subspace_solver_->has_subspace();
}

void SubspaceLambdaSolver::clear_subspace()
{
    subspace_solver_->clear_subspace();
}
...
LambdaSolverResult SubspaceLambdaSolver::run(int outer_step)
{
    ...
    // --- Lambda loop ---
    for (int i_step = -1; i_step < max_inner_iter; i_step++)
    {
        double duration = 0.0;
        auto iterstart = std::chrono::steady_clock::now();
        ...
        if (i_step == -1)
        {
            // Phase 1: Full diagonalization and build subspace cache
            ...
            if (PARAM.inp.nspin == 2)
            {
                dynamic_cast<hamilt::DeltaSpin<hamilt::OperatorLCAO<std::complex<double>, double>>*>(
                    sc_.get_operator())->update_lambda();
            }
            ...
            // Build subspace at current lambda
            subspace_solver_->build_subspace(hamilt_t, *psi_t, sc_.pelec,
                                              *sc_.dm_, PARAM.inp.nspin,
                                              sc_.get_sc_lambda());

            // Get Mi from full diagonalization
            spin = sc_.get_Mi();
            where_fill_scalar_else_2d(sc_.get_constrain(), 0, zero,
                                       sc_.get_sc_lambda(), initial_lambda);
            print_2d("initial lambda (eV/uB): ", initial_lambda,
                      sc_.get_nspin(), ModuleBase::Ry_to_eV);
            print_2d("initial spin (uB): ", spin, sc_.get_nspin());
            print_2d("target spin (uB): ", sc_.get_target_mag(), sc_.get_nspin());
        ...
        {
            // Reset delta_lambda
            std::fill(delta_lambda.begin(), delta_lambda.end(),
                       ModuleBase::Vector3<double>(0.0));
            add_scalar_multiply_2d(delta_lambda, dnu, one, delta_lambda);
            where_fill_scalar_else_2d(sc_.get_constrain(), 0, zero,
                                       delta_lambda, delta_lambda);

            // Compute new lambda = initial_lambda + delta_lambda
            std::fill(cur_lambda.begin(), cur_lambda.end(),
                       ModuleBase::Vector3<double>(0.0));
            add_scalar_multiply_2d(initial_lambda, delta_lambda, one, cur_lambda);
            sc_.set_lambda(cur_lambda);

            // Update DeltaSpin operator
            if (PARAM.inp.nspin == 2)
            {
                dynamic_cast<hamilt::DeltaSpin<hamilt::OperatorLCAO<std::complex<double>, double>>*>(
                    sc_.get_operator())->update_lambda();
            ...
            // Solve using subspace diagonalization
            subspace_solver_->solve(hamilt_t, *psi_t, sc_.pelec,
                                     *sc_.dm_, *sc_.pelec->charge,
                                     PARAM.inp.nspin, true,
                                     &cur_lambda);

            // Compute weights
            elecstate::calculate_weights(sc_.pelec->ekb, sc_.pelec->wg,
                                          sc_.pelec->klist, sc_.pelec->eferm,
                                          sc_.pelec->f_en,
                                          sc_.pelec->nelec_spin,
                                          sc_.pelec->skip_weights);
            ...
            bool grad_ok = sc_.check_gradient_decay(new_spin, spin,
                                                     delta_lambda, dnu_last_step);
            if (i_step >= nsc_min && grad_ok)
            {
                std::fill(cur_lambda.begin(), cur_lambda.end(),
                           ModuleBase::Vector3<double>(0.0));
                add_scalar_multiply_2d(initial_lambda, dnu_last_step, one,
                                        cur_lambda);
                sc_.set_lambda(cur_lambda);
                sc_.update_psi_charge(dnu_last_step.data(), true);
        ...
        if (i_step == 0)
        {
            sc_.set_current_sc_thr(
                std::max(rms_error * sc_.get_sc_drop_thr(), sc_.get_sc_thr()));
        }
        ...
        if (sc_.check_rms_stop(outer_step, i_step, rms_error, duration,
                                inner_loop_duration))
        {
            sc_.update_psi_charge(dnu_last_step.data(), true);
            result.converged = true;
        ...
        // Solve using subspace diagonalization
        subspace_solver_->solve(hamilt_t, *psi_t, sc_.pelec, *sc_.dm_,
                                 *sc_.pelec->charge, PARAM.inp.nspin, true,
                                 &cur_lambda);
        ...
        double alpha_plus = alpha_opt - alpha_trial;
        scalar_multiply_2d(search, alpha_plus, temp_1);
        add_scalar_multiply_2d(dnu, temp_1, one, dnu);
        delta_lambda = dnu;
        ...
        double g = 1.5 * std::abs(alpha_opt) / alpha_trial;
        if (g > 2.0) g = 2.0;
        else if (g < 0.5) g = 0.5;
        alpha_trial = alpha_trial * std::pow(g, 0.7);
...
LambdaSolverResult FDCGLambdaSolver::run(int outer_step)
{
    ...
    if (rms_error < current_sc_thr)
    {
        ...
        {
            sc_.update_psi_charge(nullptr, true);
        }
        result.converged = true;
        result.rms_error = rms_error;
    ...
    for (int k = 0; k < nsc; k++)
    {
        ...
        const double sccut = sc_.get_sccut();
        if (sccut > 0)
        {
            double max_step = 0.0;
            for (int ia = 0; ia < nat; ia++)
    ...
    else
    {
        ...
        sc_.update_psi_charge(delta_for_pw.data(), true);
...
std::unique_ptr<LambdaSolver> create_lambda_solver(
    ...
    SpinConstrain<std::complex<double>>& sc,
    hsolver::SubspaceMode subspace_mode,
    hsolver::SubspacePrecision subspace_precision)
{
    switch (solver_type)
    {
        ...
        case LambdaSolverType::Subspace:
            return std::unique_ptr<LambdaSolver>(
                new SubspaceLambdaSolver(sc, subspace_mode, subspace_precision));
