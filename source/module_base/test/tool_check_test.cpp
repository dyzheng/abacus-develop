...
TEST_F(ToolCheckTest, Name)
{
	...
	testing::internal::CaptureStdout();
	EXPECT_EXIT(ModuleBase::CHECK_NAME(ifs, "abacus"), 
...
TEST_F(ToolCheckTest, Int)
{
	...
	// quit check: quit = false
	testing::internal::CaptureStdout();
	EXPECT_EXIT(ModuleBase::CHECK_INT(ifs, 80), 
		::testing::ExitedWithCode(0), "");
	output = testing::internal::GetCapturedStdout();
	EXPECT_THAT(output,testing::HasSubstr("NOTICE"));
	ifs.close();
...
TEST_F(ToolCheckTest, Double)
{
	...
	// quit check
	testing::internal::CaptureStdout();
	EXPECT_EXIT(ModuleBase::CHECK_DOUBLE(ifs, 0.22998), 
		::testing::ExitedWithCode(0), "");
	output = testing::internal::GetCapturedStdout();
	EXPECT_THAT(output,testing::HasSubstr("NOTICE"));
	ifs.close();
...
TEST_F(ToolCheckTest, String)
{
	...
	// quit check
	testing::internal::CaptureStdout();
	EXPECT_EXIT(ModuleBase::CHECK_STRING(ifs, "scf"), 
		::testing::ExitedWithCode(0), "");
	output = testing::internal::GetCapturedStdout();
	EXPECT_THAT(output,testing::HasSubstr("NOTICE"));
	ifs.close();
