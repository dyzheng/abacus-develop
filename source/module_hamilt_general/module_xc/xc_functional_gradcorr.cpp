                             ...
                             const bool is_stress)
{
    ...
    assert(nspin0 > 0);
    const double fac0 = PARAM.inp.nspin == 2 ? 0.5 : 1.0;
...
#pragma omp parallel for schedule(static, 1024)
        ...
        // used for |\nabla m|
        std::vector<double> transverse_corr(rhopw->nrxx, 0.0);
...
#pragma omp critical(xc_functional_gradcorr_reduce)
    ...
    if(!is_stress && PARAM.inp.nspin==4 && XC_Functional::gga_grad==2)
    {
...
#pragma omp parallel for reduction(+ : sum) schedule(static, 256)
        ...
        if(PARAM.inp.nspin == 4 && XC_Functional::gga_grad == 1)
        {
#ifdef _OPENMP
#pragma omp parallel for collapse(2) schedule(static, 1024)
#endif
