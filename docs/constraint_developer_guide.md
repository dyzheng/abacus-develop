...
> 版本基线：feat/deltap `7bd4fd3ee`（2026-09-08）。读者：后续开发者。
...
| L1 | M2 | `constraint_observe.{h,cpp}` | 读数核 `ConstraintObserver::observe`（静态无状态） |
| L2 | M3a | `constraint_inject_pw.{h,cpp}` | PW veff 注入核 `ConstraintInjectPW::inject`（静态无状态） |
...
| L3 | M5 | `constraint_accounting.{h,cpp}` | 记账 `ConstraintAccounting`：E_con + key=value 审计行 |
| L4 | M6 | `constraint_deriv.{h,cpp}` | 力核 `constraint_force`（静态自由函数，PW/LCAO 共用） |
| L5 | M7 | `constraint_io.{h,cpp}` | 配置/JSON 解析/语义守卫 `configure_from_inputs` |
...
| constraint_target_file | （文件内容） | 见 2.2；空 → ERROR |

### 2.2 靶点 JSON → `std::vector<ConstraintTarget>`（constraint_io.h:14）

```json
{"targets": [0.1, -0.1], "atoms": [[0], [1, 2]]}
```
`targets[i].value`（delta 模式为偏移量）+ `targets[i].atoms`（全局原子下标片段）。`"atoms"` 缺省 → 逐原子。解析器是**手写 JSON 子集**（仅上述结构），非通用 JSON。
...
CONSTRAINT_AUDIT c[0] q=.. t=.. mu=.. res=..
```
...
| `constraint_force` | F_J=−Σμ Σ_g d·∂w/∂R_J ΔV（双基组同核） | deriv::ForceOnSyntheticDensity / NewtonThirdLaw / ForceLinearInMu |
...
| `configure_from_inputs` | 全部语义守卫（PW/LCAO 共享） | io::Guards / ConfigureFromInputsShared（五分支） |
| `ConstraintInjectLCAO::build/trace` | W^α_μν Gint 积分；Tr[W·DM] 审计 | inject_lcao::PartitionSumRuleEqualsOverlap（ΣW≡S，2e-15） |

## 5. 单测覆盖总账（12 ctest 目标全绿）与未覆盖点

已覆盖：全部数学核、守卫分支、双通道、MPI 一致性、记账、力核。**反向破坏验证**（sabotage ×3）证明守卫能抓回归。
**未覆盖/弱覆盖**（后续开发注意）：
...
- **通道枚举化**（DensityChannel）：charge/spin 差异收敛到枚举+分支，无字符串散落；
...
| 风险 | 级别 | 说明与缓解 |
|---|---|---|
| **时序契约隐式**：inject 必须在 HSolver 前、observe 在 iter_finish、v_eff 重建时机会抹掉 μw（§3.1）。esolver 重构（ABACUS 正在进行的 esolver 重构线）最易破坏此契约，且**无静态检查** | 高 | 缓解：新增注入点时强制走 §3.1 检查；在 esolver 钩子处保留集成冒烟（211/212 用例）作为回归网；考虑在 inject 处加"本迭代 v_eff 已被重建？"的断言式守卫 |
...
| **channel 为 run 级全局**：混合 charge+spin 需要逐约束 channel（当前 schema 无法表达，守卫拒绝） | 中 | 已知边界（混合约束 Task 2.4.1 预案：JSON 逐约束 type + 逐分量 channel 数组） |
...
| 要加什么 | 动哪里 | 必过检查 |
|---|---|---|
| 新约束类型（偶极等） | DensityChannel + M7 守卫 + 逐约束 channel（先做 2.4.1） | mock 收敛 + 反假收敛 + §3.1 势存活检查 |
| 新权重（Hirshfeld） | WeightType 枚举 + M0 核 + M1 构造 | 单位分解 + 导数 FD + 平移不变性 |
| 应力 | M6 新核（推导先行，R6 在案） | 应力 FD + Maxwell 闭合 + 势存活检查 |
| Broyden 多约束 | M4（保留逐分量作 fallback） | mock 病态映射 + 近共线对（47 步案例作基准） |
| MD/relax 接线 | 每几何 init + build + build_derivatives | Γ-path 先例（DeltaP 4.3）+ 力 FD 全绿前提 |

## 7. 文档与验证资产索引

- 测试：12 ctest 目标（`module_constraint/test/`）+ 3 集成用例（tests/01_PW/211、212，tests/02_NAO_Gamma/212）+ FD 脚本（tests/constraint_fd_force/tools/）；
