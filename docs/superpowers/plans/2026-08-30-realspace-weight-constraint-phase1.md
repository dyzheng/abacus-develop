...
- [x] **Task 10 V2 口径基准**：同一收敛密度导出 cube，Multiwfn 算 Becke 电荷对拍（消电子结构差异，纯验分账规则，判据 1e-4 e）。
  - **替代完成（V2a+V2b 内部闭环，2026-08-31，见 v2a-grid-convergence.md）**：
    V2a 三档网格（81³/120³/162³，同 34457-G 基组）相邻档差 <1e-4 e
    （max 8.2e-5）；解耦固定密度复核 81³-vs-162³ ≈5e-5。严格单调因
    切换面网格对齐振荡不成立（口径偏差已登记），操作判据=收敛带宽
    <1e-4 e。V2b 测试内独立参考（Becke 1988 原始公式重写）对拍 <1e-8
    PASS。Multiwfn 降级为可选 V2c'（约定级交叉核对，三期/有机器的场合）；
    "基组无关/口径正确"声明以 V2a+V2b 为内部依据，外部参照待 V2c'。
