...
**当前主线（统一实空间权重约束框架，2026-09-14）**：
  ...
  4. **待裁定（文档面）**：① 是否重建 `docs/parameters.yaml` + `input-main.md`
  ...
  5. 能力边界文档（`2026-08-13-deltap-capability-boundaries.md`）持续补条目。
...
## 2026-08-17 (8): Stage 4.2 三体系驻点 FD 全矩阵（hf/co/h2o_asym 21/21 PASS）

- **协议**：T3 扩展（t_Γ*=自然 Γ 冻结 → 位移腿内循环 λ 重收敛 →
  中心差 FD vs 解析力）；新脚本 `tests/deltap_fd_force/tools/run_stationary4.sh`
  （串行、OMP=1、一次一任务）。判据 |Δ|<0.0128555 eV/Å（5e-4 Ry/Bohr）。
- **结果**：hf 6/6、co 6/6、h2o_asym 9/9 全部 PASS；最大残差 0.00658
  eV/Å（h2o_asym atom1 x），判据 1.95× 裕度。x/y 平移对称分量
  ~1e-7–1e-5（噪声级），z/链轴与原子位移分量 ~3e-3–7e-3（O(λ) 响应族，
  与 T3 0.0138 同族同量级，λ*≈1e-3–2e-2 Ry）。无新未解释项。
- **⟨η⟩ 关联检查（首次兑现检验）：阴性**。co（⟨η⟩=1.67%，超 1% 经验线）
  最大残差 0.00541 与 hf/h2o_asym（⟨η⟩=0）0.00542/0.00658 同量级；
  驻点 FD 层面 ⟨η⟩ 不预测力残差偏大，残差主导为 O(λ) 项而非完备性。
- **诚实标注**：位移腿 |Γ−t_Γ*|∞≈1.7–2.7e-3，略高于名义 1e-3（内循环
...
## 2026-08-17 (10): Stage 4.4 V1（E_eff 对拍）+ V3（BN 新记账刚度复核）

- **V1（当前 HEAD 全新双通道测量）**：ΔΣγ/Δλ=+0.0529 rad/Ry（Σ3 原子、
  raw、冻结）、ΔΣγ/ΔE=+0.6303 rad/a.u.、μ₀=−dE'/dE=0.763 e·Bohr=1.940 D
  （实验 1.855 D，5%——与 D2 逐位互证）。**符号钉死**：两通道 Σγ 响应同号，
  公式 (b) 正号正确。**因子**：E_eff(μ-lever)=0.1678 a.u./Ha = 公式 (b)
  0.1047 的 **1.60×**（与 D2 ×1.6 打印标注互证）；"10% 内"预言不成立 =
  V1 条款"误差=代理差距"（F-2b 非极化几何通道）。
- **V3（新记账 γ-drive 重采样）**：BN 9 点 γ 直接约束 + escon=−λ·Γ：
  E' span **1.72 μRy**（旧轮 2.0）、λ~1e-5 Ry、κ 拟合 13 μRy/rad²（SCF
  能量量化噪声级）→ **BN 零刚度结论在新记账下不变**（对照 h2o1 60
  Ry/rad² 差 6 个量级）。口径说明：锚点 #3 proxy-drive 数据（λ~4e-3 Ry、
  E' span 536 μRy）是 κ=1 t_Γ 猜值的 Γ-约束 λ-work 伪曲率，非 γ-PES；
...
## 2026-08-17 (12): 评审收尾两条文档修订（⟨η⟩ 使用建议 + LCAO/PW 无 target 语义分歧）

- **⟨η⟩ 使用建议按 4.2 阴性结果修订**：progress-summary §5 与能力边界
  LIMITATION 新增 L13——⟨η⟩ 保留为 SMO 完备性结构指标；其 1% 经验线
  **不预测力残差**（CO 1.67% 最大残差 0.00541 vs ⟨η⟩=0 体系
  0.00542/0.00658 eV/Å，同量级），不作为力精度预警依据。
- **LCAO/PW 无 target 语义分歧用户可见**：用户手册 §2.7 新增警告行——
  LCAO 无 target = 自由跑（λ≡0，4.3 修复后）；PW 无 target = 约束 γ→0
  （历史语义保留，`deltap_pw.cpp` 显式文档化）；跨基组对比/迁移前务必
...
## 2026-09-09 (5): Task A6 严格评审——G6 通过，A7 解锁（无代码改动）

- 亲自复跑 213 混合用例：双通道 CONVERGED（res_c=−1.38e-5/res_s=−7.86e-5）、
  μ_c=−0.1811730734/μ_s=−0.08154434822 逐位一致、15 外步、maxdev=2.2e-16、
  FINAL_ETOT 与 result.ref **逐位相等**；ctest 11/11；注册 CASES_CPU.txt:131。
- **P3 悬案结案**：跨类型耦实测非可忽略（μ 偏移 −0.0048/−0.0092 Ry 同向变负
  =互致刚化，外环 3→15 步退化），阶段 B 立项数据到手；nspin 效应 +1.9e-4
  排除法干净。
- 卫生项：用例目录运行残留（OUT.autotest/log.txt 未入库）应清理；A7 文档
  改动已在工作树（内容准确）随 A7 入库。
- 裁定：通过，阶段 A 工程任务 7/8 完成，批准 A7 文档收尾。
- 文件：`2026-09-09-taskA6-review.md`。
...
## 2026-09-09 (6): Task A7 + 阶段 A 收口评审——阶段 A 正式闭合（8/8）

- 核实：d1a246fc6 已推 zdy；手册/开发者指南/总结文档 diff 与实测行为逐条
  一致；计划 A7 勾选如实；卫生项闭环（评审者清理 213 用例目录未跟踪运行
  残留）。
- 阶段 A 整体评估：范围纪律满分（偶极/Broyden/松紧 SCF 零抢跑）；G1–G6
  无带病前进；五评审修订（P1–P5）全部落实；诚实文化保持（偏差/中间态/
  弃用恒等式均如实登记）。
- 开放项在案：混合力 FD 验收轮（下一个重型验证）/只读观测口/torque E'/
  DeltaSpin 对等/阶段 B 立项评审。
- 文件：`2026-09-09-stageA-closure-review.md`。

## 2026-09-09 (25): 下一阶段计划——混合力 FD 验收轮 + 工具补件 + 阶段 B 立项（无代码改动；原临时编号 (7)，按链式惯例顺延至 (24) 之后）

- 战略排序（先验后开发）：V1 单自旋通道力 FD（212 体系，从未过 FD 的
  缺口）→ V2 混合力 FD（213 体系，双通道冻结 t*；脚本混合适配 ~30 行，
  raw-E 红利：多约束无需逐分量修正）→ V3 工具补件（torque E' 修复 /
  只读观测口——一口两用：严格 PW≡LCAO 对拍 + DeltaSpin 密度 Becke 矩
  测量/两个开放项结案）→ V4 阶段 B 立项评审（数据已齐：A6 μ 耦合表 +
  47 步/15 步；P1 修正=不按 kind 分块，改动态分组或近奇异检测）。
- 判决门 G-V1..G-V4；纪律：判据不豁免、raw-E 口径、净力/补偿前力标准
  检查、重型跑对干净 commit。时间线约 4 天（含机时）。
- 文件：`docs/superpowers/plans/2026-09-09-mixed-force-fd-and-stageB.md`。

## 2026-09-09 (8): V1 中期评审——裁定"全量跑完"（默认路径）

- V1 Step 1/2 核实：脚本支持 commit e46b75673 在案（CASE 继承 nspin/type、
  TEST_FORCE、std_checks 自动化=2.7 纪律项制度化、legacy 字节级回滚自测）；
  O-z 冒烟 |d|=0.0034698 PASS（3.7× 富余，与 PW 电荷通道 O-z 0.00433 同噪声
  族）；净力 +0.0154 vs 修复前指纹 +11.62——自旋通道无 missing-μw 签名；
  打印力 Σz≈0 补偿后闭合（评审抽查）。
- **路径裁定：默认（全量 18 腿跑完），不批 trim**。理由：自旋通道首次力 FD
  应享全轴覆盖（先例：电荷通道首验全 18 腿；减轴只在接受机制级根因+最坏轴
  后用于 LCAO）；trim 不产 PASS 只省机时，人机成本不变；冒烟富余 3.7× 非
  豪华，全轴数据巩固包络。
- 效率发现入档（V2 用）：每腿 2 外步因 step_max=0.05<|μ*|≈0.072 无热启动——
  **fixed-μ 腿与重优化-μ 腿的 FD 已证逐位同值（SCC 修复文档，PW/LCAO 双证）**，
  V2 混合 FD（nspin=2 成本×2）建议用 fixed-μ 协议 + 1 条对照腿验证，省 ~50%
  机时；V1 中途不换协议（与冒烟可比性）。

## 2026-09-09 (9): FD 计算成本分析——非算例非算法缺陷（无代码改动）

- 成本结构实测（V1 冒烟 base，R7 nspin=2）：HSolver 对角化占 **99.9%**
  （106×11.1 s），约束注入/读数 <1%（计时表不出现）——算法缺陷排除。
- 三因素：A）**全部验证跑在 debug -O0 二进制上**（CMakeCache 实测：
  CMAKE_BUILD_TYPE=debug）——BLAS 密集对角化 -O0 vs -O3 差 3–10×，单项
  收益最大（18 腿 8.7 h → 1.5–3 h），此前无人指出（评审同责）；B）R7 网格
  是刻意验证成本（npwx 96k vs 生产 8.6k），可用网格-噪声标定降档省 3–4×；
  C）FD 腿无热启动，fixed-μ 已证逐位同值，省 50%。
- 行动：V1 本批 debug 跑完（腿间可比性）；V2 起 release（-O3）+ fixed-μ +
  噪声标定，混合 FD 预估 ~16 h → ~1–2 h；纪律新增：重型跑 spec 记构建类型，
  评审加"构建匹配"检查项。
- 文件：`2026-09-09-fd-cost-analysis.md`。

## 2026-09-09 (10): FD 验证算例重设计（纸面，零计算提交）

- 用户三点修正成立（LCAO 快/bulk PW 快/dav_subspace 快），前版成本分析
  补三维度：基组（LCAO vs PW，~10–20×）、体系（bulk 无真空，~10–30×）、
  求解器（dav_subspace vs CG，~2–5× 估计未经实测）。
- 核心设计：验证对象×最便宜载体匹配——通道级全轴 FD 放 **LCAO H₂O**
  （整腿 ~2 min 实测基准，18 腿 40–70 min）；PW 嵌入路径只要冒烟 2 轴
  （bug 模式已根因，净力指纹兜底）；bulk 的正确位置=V6 生产表征不挤进
  FD 轮。总成本 ~17 h → ~2.5–4 h 覆盖不降。
- 当前 V1 PW 18 腿处置建议：跑完 wave 3（H1-x）后停（4 轴已超冒烟集，
  余 wave 属冗余，处置权在用户）。
- 诚实标注：dav_subspace 收益未实测（首批标定腿）；冒烟充分性依赖净力
  指纹判别力，见签名即升全轴不豁免；release 切换需同构建重生成 ref。
- 流程教训：先批准再跑（前一轮误启动 benchmark 被叫停）。
- 文件：`2026-09-09-fd-case-redesign.md`。

## 2026-09-09 (11): 真实体系验证算例集设计（纸面，零计算提交）

- 用户澄清：要 H₂O 之外的物理场景算例（离子型/过渡金属氧化物等）。
- 资源摸底：FeO 与 Fe 基 DeltaSpin 用例在库（17_DS_DFTU/11/12，自旋对照
  零搭建）；Fe/Ni/Ti/Pt/Na/Cl/Mg 赝势在库；缺口=Au（V6）、Mn、C-PBE。
- 算例集四组：I 离子型（MgO/NaCl 体相宽 q 扫描=R4 可达域体系依赖的判决
  实验 + NaCl 离子对电离极限熔断）；II TM 氧化物（FeO 自旋约束 +
  DeltaSpin 同体系同靶点对照=开放项④正确结案方式，NiO 缓）；III CT 体系
  （H₂O 二聚体起步的混合约束 + Marcus 重组能 CDFT 对标，资源前提 C-PBE）；
  IV V6 bulk（Au 赝势缺口，换体系或外部引入待裁定）。
- 执行顺序 P0=II-1 FeO + I-1 MgO；每算例独立参照物必备；力 FD 不每体系
  重复 18 腿（选 1 便宜体系做增量）。
- 文件：`2026-09-09-validation-case-suite-design.md`。

## 2026-09-09 (12): P0 算例逐步实施计划（纸面，零计算提交）

- 产出 `docs/superpowers/plans/2026-09-09-p0-cases-feomgo.md`：
  I-1 MgO 宽电荷扫描（R4 判决：离子体系线性可达域是否 ≥2× 于 H₂O；
  LCAO 8 原子晶胞、网格标定先行、O 片段 8 点+Mg 片段 ±1.0 物理熔断；
  1–2 h 预估）与 II-1 FeO 自旋约束（继承 17_DS_DFTU/11 基线最小改动；
  DFT+U+约束同开兼容性首测；S5 DeltaSpin 同靶点对照；II-1b 口径测量
  依赖 V3b 只读观测口，拆出不阻塞；2–4 h 预估）。
- 顺序：先 I-1（零资源缺口、问题最核心）后 II-1；μ/κ 数据汇总进阶段 B。
- 批准前不提交任何计算。
- 文件：`docs/superpowers/plans/2026-09-09-p0-cases-feomgo.md`。

## 2026-09-10 (26): Task V1 单自旋通道力 FD——脚本自旋支持 + O-z 冒烟 PASS（PW 18 腿进行中）

- 缺口确认：2.6 的 18 腿力 FD **全部是电荷通道**，自旋通道力（M6 spin 折叠）
  只有单测覆盖，从未过 FD；本轮为 G-V1 补上判决。
- 脚本改动（唯一代码改动，`tests/constraint_fd_force/tools/run_constraint_fd.sh`）：
  `CASE=` 选例；nspin/constraint_type/nelec 由测试例 INPUT 数据驱动（自旋例→
  nspin 2 + spin）；`TEST_FORCE=1` 追加 `test_force 1`；新增 `std_checks()`
  把 2.7 标准检查（Σ CONSTRAINT 力、Σ 补偿前力、compen、分项/总力自洽）自动化。
  **回退自测：`write_input()` 改动前后在 delta/absolute 两模式下 INPUT 逐字节相同。**
- 冒烟（ONLY=0_2，np4，10:39:51→11:44:45）：t\*=0.09999858905 μB、
  μ\*=−0.07188979607 Ry（212 例 ecut=20 参考 −0.07234，同量级同符号）、
  E0=−466.9021000194886710 eV；**O-z：F_FD=−0.72247438 vs F_ana=−0.72594423，
  |d|=0.0034698 < 0.0128555 → PASS**（3.7× 富余）。
- 标准检查：Σ 补偿前净力 z=+0.0154 eV/Å（电荷通道修复后参考 +0.0089、修复前
  +11.62）→ 自旋通道无 μw-Pulay 缺失特征；Σ CONSTRAINT 力 z=−0.024 eV/Å
  （电荷通道同项 +6.19，本身非平移不变量，记录为诊断量）；分项/总力自洽
  max|pre−compen−printed|=1.75e-07 eV/Å。
- 状态：PW 18 腿（MAXJOBS=3）北京时间 11:5x 起跑，预期 ~5 h；结果待补。
- 文件：`docs/superpowers/specs/2026-09-10-taskV1-spin-force-fd.md`；commit e46b75673。

## 2026-09-11 (27): I-1 MgO 宽电荷扫描——R4 判决实验通过（线性区 ≥ ±0.8 e ≈ 2.7× H₂O）

- 计划：`docs/superpowers/plans/2026-09-09-p0-cases-feomgo.md` 算例 I-1；
  本轮 S1–S5 全跑完，实盘目录 `tests/deltap_mgo_scan/`
  （driver `run_mgo_scan.sh` + `results/summary.txt` + 13 份 `running_scf.log`）。
- **判决（核心）**：MgO 单 O 电荷约束的**线性区 = −1.0 … +0.8 e**（负侧局部
  斜率全程偏离参考 κ 仅 −4.0%，正侧到 +0.8 e 为 +12.2%，仅 +0.8→+1.0 段
  越 20% 到 +23.0%）→ **≥ ±0.8 e ≈ 2.7× H₂O 的 ±0.3 e**，满足立项的
  "≥2×" 判据。设计方案 §1.4 "DeltaQ 正当适用域 = 离子/缺陷体系" 得到判决
  实验支持（**正向**结论，非否定）。
- 机制修正（比原假设更精确）：宽可达域**不是**因为离子体系更软——O in MgO
  的 |dμ/dδ| = 1.9–2.3 Ry/e 与 H₂O 的 1.7 Ry/e 同量级且**更硬**；真正原因
  是离子体系在大扰动下**能量曲面保持近二次**（共价体系 ±0.3 e 外即偏线）。
  且有强不对称：O 减电荷（1.90）比增电荷（2.53）软 ~25%。
- **bulk 分区首证**：双完整子晶格（O 4 + Mg 4）覆盖下 `total_charge = 64
  = nelec` 逐位成立、maxdev = 6.66e-16；Becke 电荷 Mg +1.4427 / O −1.4427 e
  落在文献带 ±1.0–1.5 e 内。片段可加性（单原子 vs 子晶格/4）一致到 1.3e-8 e。
- **独立校验**：网格交叉点（δ=+0.8 e：60/240 vs 80/320）μ* 差 **0.018%**；
  能量恒等式 μ*(中点) = −dE_tot/dδ 在全部 8 个区间偏差 **<0.8%**；δ=+0.5
  两次独立运行 μ* 逐位相同（−1.049024359）。
- **计划预期被推翻**：Mg 片段 −1.0 e（Mg³⁺ 化）**未熔断**（μ*=+2.115 Ry
  ≪ 5.0 Ry 顶限，Q 精确达标）→ 本算例没有产生物理熔断用例；若需熔断语义应
  推到 −2…−3 e（下一轮可选）。
- **协议/成本成果（II-1 可直接复用）**：① LCAO 下 `ecutrho` 必须 = 4×`ecutwfc`
  （ABACUS 硬校验）→ 网格标定只有一个旋钮；② 网格标定判据不能用"Q_ref 绝对
  稳定 <thr/3"（bulk 最优仅 ~3e-4 e，200/800 还因内存被杀），改用"μ/κ 网格
  稳定性 <1%"；③ 外环 `MuSolverParams::step_max = 0.05 Ry` 硬编码 → 外步数
  = |μ*|/0.05，`scf_nmax` 必须 ≥ 数百（计划原值 200 不够，δ=+1.0 e 用了
  788 迭代）；④ Release 二进制与 debug 在 60/240 与 160/640 上**逐位一致**
  （可安全切换，但只快 ~1.3×——真正杠杆是网格 11 s vs 57 s/自由 SCF）；
  ⑤ 热启动链（`init_chg file` + `read_file_dir`）生效且不污染参考态。
- 成本：S1–S4 ≈ 2.5 h（np4，含并行；生产网格 60/240 后单点 3–8 min）。
- 文件：`docs/superpowers/specs/2026-09-11-i1-mgo-charge-scan.md`（本轮 spec）、
...
## 2026-09-11 (3): DFT+U 与统一约束框架算法冲突评估（无代码改动）

- 结论：**无正确性层面冲突**——势通道共存（FeO 实证）与自洽结构均无冲突；
  真正的冲突在观测量语义层：约束共轭于 Becke 加权矩，U 校正 on-site d 占据，
  体系存在"廉价尾部通道"绕过 U 的校正目标（本轮数据：良态跟随 74%、换态
  反向脱钩）——CDFT 观测量选择的固有权衡，非 bug。
- 处置链在案：onsite= 仪器（1c1199987）+ L14 边界（04f6b8482）+ 半径敏感性
  （4b 待锚点）+ on-site 权重类型只能阶段 C 明示权衡（基组口径分裂回归代价，
  不得静默引入）。
- C-29 揭示的更一般缺口：外环熔断假设"外步=收敛 SCF"，SCF 不收敛时 μ 顶限
  熔断实际不可达且收尾路径堆破坏（预存在）——除 C-29 定位外，必须补
  "SCF 不收敛→熔断/降级"路径（即 L15 盲区②解法）。优先级：C-29 定位 >
  SCF 不收敛熔断 > 4b > S4/S5（继续暂缓）。
- 文件：`2026-09-11-dftu-constraint-conflict-evaluation.md`。

## 2026-09-11 (4): 实空间权重实现 DFT+U 的公式级再分析（无代码改动）

- 应要求做算符级推导（`2026-09-11-realspace-dftu-derivation.md`）：
  推广式 $E_U^{(w)}=\frac U2\sum_\sigma[\mathrm{Tr}(\hat w\hat n)-\mathrm{Tr}((\hat w\hat n)^2)]$
  在幂等极限精确退回 Dudarev（结构验证）；能带形式只需求带矩阵
  $W_{ij}=\langle\psi_i|\hat w|\psi_j\rangle$（对角元=框架已有的逐带加权布居）；
  势 $V_{ij}=U[\frac12W_{ij}-\sum_k f_kW_{ik}W_{kj}]$ 与 Dudarev 逐项同构。
- 三个物理警告：①模糊窗使完全占据态也读"分数"→$W\to1$ 的盆地内压驱动，
  相对标准 U 可能过度局域化、U 值必须重定标；②共价区幂等冲突（键合电子
  固有分数占据）→适用域不宽于标准 DFT+U；③相关子空间从 d 壳变模糊盆地，
  是不同有效模型；双计数问题照旧。
- 协同（真实吸引力）：U 校正量与约束控制量同观测量（廉价尾部通道消失）、
  PW≡LCAO 投影子依赖消除、外环 Jacobian 即 Cococcioni 线性响应定 U 机器。
- 裁定：研究方向成立（阶段 C/D），**不是 DFT+U 替代品、也不解决 II-1b**
  （观测量选择问题）；先做 V-d 文献核查（防重复造轮子）+ V-a 诊断量
  $\mathrm{Tr}((\hat w\hat n)^2)$ 趋势对照（低成本），再定原型投入。

## 2026-09-11 (5): 双迭代策略（λ 外环-SCF 内环 vs SCF 外环-λ 内环）计划（纸面）

- 用户需求：两种 μ 更新调度并存 + 定量对比；聚焦统一约束框架，DFT+U 算例
  暂缓。
- 计划 `docs/superpowers/plans/2026-09-11-dual-iteration-strategy.md`：
  OUTER（现状默认逐位不变）+ INNER（DeltaSpin lambda_loop 式：drho 门控
  内环更新 + mix_reset + inner_nmax）+ **Settle check**（防"内环收敛而弛豫
  反弹"历史陷阱，2026-07-20 教训）+ 反假收敛 T4a 纪律。
- 历史裁量：DeltaSpin 内环生产先例支持用户假设；T-7″ 否决是 γ 观测量
  特有（电荷/自旋冻密度响应=χ₀ 负且单调，不适用）；mixing 破坏（用户
  担心点）不测不判——mix_reset 默认开 + 复位代价列为测量对象。
- 定量研究：6 体系×2 策略对照矩阵（H₂O 三例基线/MgO/FeO 双稳/Mg 远侧），
  主指标=总对角化次数，正确性等价判据（μ* 1%、E 1e-6），mixing 机制专项
  Q3，产出"何时用哪个策略"决策表入手册。
- 工期 ~3–4 天；M4 求解器不动（仅调度，Broyden 属独立后续）。
...
## 2026-09-14 (6): 元数据修复评审 + 物理测试就绪性评估（无代码改动）

- 31322f2fa 通过（范围纪律正确、--help 验证、56/56）。
- 两待裁定项批复：①parameters.yaml 重建**不授权**——对外契约不应在
  28+12 个开发态参数语义未冻结时发布，正确时点=merge-readiness 评审；
  ②spin.md DeltaSpin 参数表刷新批准（独立 docs 小 commit）。
- **物理测试就绪性裁定：可开始 SCF 级物理（不依赖力）**——起步组合：
  III-1 H₂O 二聚体 CT+Marcus（旗舰 CDFT 接口）+ I-1 扩展 + Fe 基 DeltaSpin
  交叉验证；两道闸门：力相关物理需补完 V1（**日志核查发现自旋力 FD
  18 腿自 09-10 开跑后无完成记录=未闭合**，建议按重设计处方 ~1–2 h 补完），
...
## 2026-09-14 (10): 总览/V1 对账入库 + 成本分析更正注记 + V1 补跑入近期队列

- 用户批复（评审件 `2026-09-14-summary-and-v1-review.md`）三项：
  1. 三项改动入库 → **`74eff461b docs(constraint): summarize completed tests and results`**；
  2. 成本分析更正注记（小件，并入后续 docs commit）；
  3. **V1 补跑**（重设计处方：fixed-μ + LCAO 载体全轴 + PW 冒烟）列入近期队列——
     它是"力相关物理测试"的**唯一闸门**。
- 评审核实（与我的总览口径一致）：状态表覆盖 09-07…09-14 全部已归档轮；抽查数据
  （μ 耦合 Δμ_c −0.004819/Δμ_s −0.009206、双迭代数值、C-29、力 FD 判据）与历轮记录一致；
  V1 有效结果只剩 O-z 1/9 轴；base 跨二进制对账证明 base 可复现；4 条流程纪律与评审链一致。
- **成本分析更正**（`2026-09-09-fd-cost-analysis.md`：文首指针 + 新增 §5）：
  - V1 对账实测 release vs debug 同 base = **1422 s → 1247 s（1.14×）**，
    §2 因素 A 的"3–10×"预测被**否决**；
  - 根因：DiagoCG 耗时主体在 **BLAS/FFT 厂商库**（-O0/-O3 都是优化编译），
    `-O0` 只影响占比很小的非库代码 ⇒ "debug 构建是最大成本因素"的判断不成立；
  - 修订杠杆排序（实测/半实测）：**LCAO 载体（~10–20×）> 求解器（`dav_subspace`）>
    fixed-μ（2×）> 网格标定（3–4×）> 构建类型（1.14×）**；
    §4 建议行动改为 "V1 补跑 = fixed-μ + LCAO 全轴 + PW 冒烟"。
- **队列更新**：V1 补跑升为**近期队列第 1 项**；前置 = 给 `run_constraint_fd.sh` 加 `RESDIR`
  归档（否则重蹈 `/tmp` 丢失）；4b/II-1/阶段 B 顺延。
- 文件：`docs/superpowers/specs/2026-09-09-fd-cost-analysis.md`（§5 + 文首指针）、
  本文档（Spec Index 行 + 队列重排 + 本条）；入库 `docs(review): fold summary/V1 review + cost-analysis correction`。
- 同轮（V1 补跑前置）：runner 归档修复 + LCAO spin 载体，入库
  `test(constraint): archive V1 FD leg results + LCAO spin carrier + basis-agnostic std checks`。
