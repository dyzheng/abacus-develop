...
| 混合通道（charge+spin）力 FD | LCAO 全轴 9/9 | **进行中 6/9 轴全 PASS**（max \|d\|=1.554e-3 O-z / 8.3×；H1-z 8776×）；v2 需重优化 μ（fixed-μ 是标量），每腿 16–27 min | ⏳ 交接续跑 | `2026-09-15-mixed-channel-force-fd.md` |
...
3. **混合 charge+spin 同原子/同 run**：可用，μ 耦合已实测；跨类型耦合不可忽略（阶段 B 依据）。
   **力 FD 进行中**（LCAO 6/9 轴已 PASS，见 `2026-09-15-mixed-channel-force-fd.md`）。
...
1. **混合通道（charge+spin 同原子）力 FD**：**进行中**——LCAO 6/9 轴已 PASS，
   剩余 3 轴在另一台机器续跑（命令见 `2026-09-15-mixed-channel-force-fd.md` §5），其后 PW 冒烟 2 轴
