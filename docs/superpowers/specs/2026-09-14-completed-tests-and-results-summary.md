...
**不在本文档范围**：未跑完的轮（V1 自旋力 FD 18 腿）、被闸门挡住的物理
（约束 relax/MD、TM d 矩物理解读）、以及用户明确暂缓的项（FeO 双稳对照）。
...
| V1 自旋力 FD | 9/9 轴 PASS | **仅 O-z 1/9**（PASS）；18 腿两轮开跑均无归档 | ❌ 未闭合 | `2026-09-10-taskV1-spin-force-fd.md` |
...
### 3.11 V1 自旋力 FD（未闭合，唯一未归档的测试轮）

- **协议**：载体 `tests/01_PW/212_PW_constraint_h2o_spin`（nspin=2，δ=0.005 Bohr，
  ecutwfc=100/ecutrho=400/scf_thr=1e-8），绝对模式**冻结 t\***、**raw-E 口径**，
  判据 0.0128555 eV/Å。
- **已得结果**：

| 项 | 值 |
|---|---|
| O-z（2026-09-10，Debug） | F_FD = −0.72247438 vs F_ana = −0.72594423 eV/Å，\|d\| = **0.0034698**（3.7× 富余）→ **PASS** |
| 净力标准检查 | Σ 补偿前 z = +0.0154 eV/Å ≈ 0；**自旋通道无 μw-Pulay 缺失特征** |
| base 跨二进制（release 复跑） | μ* = **−0.07188979603 Ry**（vs debug −0.07188979607，**一致到 4e-11**）；F_ana(O-z) 差 7.2e-6；std-check 1.750e-07 逐位复现 |
| release 提速 | **仅 1.14×**（1422 s → 1247 s），远低于 cost-analysis 的"3–10×"预期 |
| 18 腿 | 两轮（09-10 / 09-14）开跑后**均无归档**；本轮工作目录 `mktemp -d /tmp/cfd_pw_XXXX` 随会话清理丢失 |

- **G-V1 状态：未闭合（仅 1/9 轴）** ⇒ **力相关物理（约束 relax/MD）维持锁定**；
  TM d 矩解读仍需 II-1b/4b 先收口。
- **根因是流程不是物理**：`tests/constraint_fd_force/tools/run_constraint_fd.sh:70` 用
  `/tmp` 临时目录且不归档进 `results/`（新脚本 `run_inner_thr_scan.sh`/`run_ct_scan.sh` 则归档）
  ⇒ 长跑一中断计算全废。这是 09-10"无完成记录"的同一坑。
...
2. **spin 约束**：PW/LCAO 读数-注入算符同构；**能量-导数一致仅在 O-z 单轴验证**（V1 1/9）
   ⇒ 力/几何相关用途**不可声明**；物理解读只能用"Becke 加权矩"口径（≠ d 局域矩）。
...
| 闸门 | 阻塞的物理 | 缺口 | 成本 |
|---|---|---|---|
| **V1 自旋力 FD** | 约束 relax / MD、几何驱动、自旋通道力/应力 | 8/9 轴未跑；跑批不归档（先修 runner） | ~1–2 h/轮（PW），LCAO 更省 |
...
1. **跑批产物必须落仓库**：长跑脚本一律 `RESDIR` 归档（README + `results/*.audit` +
   `summary.txt`），禁止只留 `/tmp`（V1 两轮作废的直接原因）。
...
1. **V1 runner 加 `RESDIR` 归档**（1 行级），然后重跑 18 腿（建议 LCAO 载体，PW base 已有）
   → 过闸门 1，解锁力相关物理；
2. **III-1 加密** δ ∈ [−0.05, +0.10]（0.01–0.02 e）→ 判定 δ≈0 拐点性质，再谈 Marcus 重组能；
3. **4b 半径敏感性 / II-1 重锚定**（合批）→ 收口闸门 2；
4. **阶段 B 立项评审**（输入已齐，见 §4.3）；
5. **merge-readiness**：届时统一重建 `docs/parameters.yaml` + `input-main.md`（接口冻结后）；
6. FeO 双稳对照**维持暂缓**（用户批复）；V6 Au:Si 待资源裁定。
