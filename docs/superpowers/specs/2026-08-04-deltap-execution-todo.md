...
| **T-18** | **V-H3' Ô_w 判决** | dγ/dλ 实测（预言 ≥3 rad/Ry，对照 proxy 的 0.3）；γ 直驱驻点 FD @ t_γ=0.98γ_nat | **γ-hold FD 残差 < 0.02 eV/Å**（对照 proxy 的 0.502）——Ô_w 成立的判决点 | ⏳ **已解锁**（T-17 h2o1 全窗收敛；BN 既有限制不影响 h2o1 判决） |
...
（S2 立项）。**T-18 已解锁**：γ 直驱驻点 FD @ 0.98 靶点（dγ/dλ 实测 ≥3 预言）。
