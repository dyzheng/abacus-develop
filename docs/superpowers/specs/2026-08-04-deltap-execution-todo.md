...
| F-4 | T-7'' 内循环解锚 | 驱动 raw γ（compute_gamma_raw 已接线），**验收改 0.005 rad 小靶点**（λ 预算内可达）；raw↔锚定偏移退出交接需显式记账 | 残差单调下降 + γ 真实移动 + 收敛 |
...
| F-8 | L2 应力（H_HK 应力 → V-H7；PW 应力） | RouteA++ 文档 §4 推导现成 |
