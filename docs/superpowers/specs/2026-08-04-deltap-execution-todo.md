...
## ★ 当前 TODO（2026-08-07 调整后，以此节为准）

> 依据 `2026-08-07-deltap-route-a-plus-plus-review-and-plan.md`（三层方案）调整。
> 原 Stage 3-5 顺延；T4b 并入 V-H3；锚点第 4 次重建推迟到 L3 全绿后一次。

| # | 任务 | 内容 | 验收（参照物） | 状态 |
|---|------|------|----------------|------|
| **T-1** | **Phase 0.3-lite 分支锚定门控**（先做，~0.5 天） | INPUT `deltap_branch_anchor=continuity\|target`：operator 模式默认 continuity（W_prev_/branch.dat 连续锚 + 冻结位移读数），gamma 旧模式锁定 target（一行行为不变）；plumbing：INPUT → module_deltap 分支选择（deltap_wannier.cpp:1288+）门控 | ① gamma 模式零回归（逐字节）；② operator 模式 t_γ=自然值时两锚一致（no-op 对照）；③ λ=0 自由首测报告**自然 γ**（T4a 伪收敛消失） | ✅ 实现完成；③ ref2/T4a 实测 ✓；① 门控锁定未跑 BN 对照（基线存档无效，见 2026-08-09 文档） |
| **T-2** | **T4a' 外循环机制验证**（~0.5 天） | t_γ=0.98γ_natural，固定几何 secant（deltap_outer_nmax 驱动，T4a 前置代码已在工作区） | 预言：2–4 步 \|γ−t_γ\|<1e-2；终点 λ*≈0.37 Ry；**γ 轨迹全程连续**（无 >量子/2 跳变）；κ 实测（≈14）更新 | ❌ **偏离即停**（2026-08-09）：frozen 读数/自然 γ/轨迹连续/κ=12.3 实测 ✓，但外循环发散（\|γ−t\| 0.110→0.225）。根因三层钉死：内循环 max_step=0.005 饿死 λ 预算、单 α 线搜索无法驱动反号分量、**H 原子 Γ-proxy 退化**（dΓ_H/dλ_H≈0）。0.98 靶点超出 Route A+ 控制域（线性窗 \|Δγ\|≲0.03–0.05/原子）。详见 `2026-08-09-deltap-t4a-frozen-shift.md` |
| **T-3** | commit（T-1+T-2 + T4a 前置代码 + 文档） | 含 Phase 0.3-lite、外循环驱动、限幅/κ 修正 | CI 式回归：单测 17/17（11+6）、MPI smoke 3/3 | ⏳ 待提交（T-2 判决失败但根因已文档化；max_step=0.1 实验已回退） |
| **T-4** | **L1.1 PW Γ 记账**（~0.5 天） | escon^PW = −Σλ_I·Γ_I^PW，Γ_I^PW=⟨P̂_I^onsite⟩（RouteA++ 文档 §5；dspin becp 机制现成） | PW T2 类比：∂E'/∂λ 从 O(1) → O(λ)（h2o_pw λ 扫描）；PW 1-rank A/B 旧模式零回归（开关保留） | ☐ |
| **T-5** | **L1.2 ⟨η⟩ 完备性输出**（~0.5 天） | scf 收敛后打印 ⟨η⟩=1−Σ_I w_In 的带/k 平均 | H2O ⟨η⟩<1%（预言）；打印进 [DeltaP P3] 区 | ☐ |
| **T-6** | **L1.3 spread_I 诊断输出**（~0.5 天） | spread_I = [Σ_n w_In(θ_n−θ̄_I)²/Σ_n w_In]^½（§1.4 推论 1，仅测量） | 输出正确性：均匀 θ 时 spread=0 的合成检验 | ☐ |
| **T-7** | **V-H2/V-H5 数据 + A+ 适用域文档**（~1 天） | ⟨η⟩：H2O vs BN 定标；3–5 个 spread 梯度体系的 A+ 力残差相关性 | spread_I–残差单调拟合落档；适用域判据写入用户文档 LIMITATION | ☐ |
| T-8 | Stage 3 hk MPI 修复（生产面，不变） | 本地列→全局带映射；串行逐字节硬约束 | hf/co corr=1 4-rank 与串行一致 | ☐ |
| T-9 | L2.1/L2.2 H_HK 应力 + PW 应力 → V-H7 | RouteA++ 文档 §4 推导，复用重叠应力内核 | 静水压+剪切 FD <1% | ☐ |
| T-10 | L3.1 Ô_w（编译开关 proxy\|efc）→ V-H0/V-H8/V-H3 | θ_n·P̂_I 替换 τ_α；D2 约束：θ 初始化锚定连续值、跳变冻结 | V-H0：Tr[ρÔ_θ]=0 待 Ô_θ；Ô_w 先做：V-H8 无极限环、V-H3 η/λ* 实测落档 | ☐ |

**立即执行：T-1（Phase 0.3-lite）→ T-2（T4a'）→ T-3（commit）。**
依赖：T-4~T-6 互不依赖可与 T-8 并行；T-7 依赖 T-5/T-6；T-10 需 T-3 后
（基线干净）+ D2 设计约束评审通过。
