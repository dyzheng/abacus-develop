...
| **T-7''** | **ow γ-drive 内循环解锚**（T-7' 机制级修复） | 内循环驱动可观测量改 raw γ（`compute_gamma_raw` 已接线）或冻结 branch-shift 读数，弃用锚定报告 | ow a2 两臂（cg/jacobi）内循环收敛（rms<1e-3）；T-18 0.98 判决受 \|λ*\|_max≈0.4–1.6e-3 Ry 工作窗限制 | ☐ |
