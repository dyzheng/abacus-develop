...
| **T-6'** | **L3.1 Ô_w 提前**（评审已裁） | θ_n·P̂_I 替换 τ_α（开关隔离 + D2 跳变冻结约束）→ V-H8（SCF 稳定）+ H 可达性复测 | 判决：γ_H>自然 是否变得可达 | 🔄 **进行中**（T-11~T-16 ✅ 落地 + R5 PASS 信号；T-17 V-H8 ❌ 极限环定位完成——下一主线 H_ow SCF 稳定化，见 T-6' 修复节） |
...
| **T-17** | V-H8 SCF 稳定性 | ow 模式 h2o1 + BN：drho 无极限环（单调入 1e-6）；跳变冻结触发后驱动行为正确 | 两体系收敛；冻结-恢复轨迹落档 | ❌ **FAIL（定位性）**：ow λ=+0.01 150 iter 极限环（Γ/escon 稳定、drho 不降、D2 冻结零触发 → 状态依赖 H_ow 刚度问题，非分支跳变）；proxy 同参数 27 iter 收敛 → ow 特有。**H_ow SCF 稳定化为 T-6' 下一主线** |
| **T-18** | **V-H3' Ô_w 判决** | dγ/dλ 实测（预言 ≥3 rad/Ry，对照 proxy 的 0.3）；γ 直驱驻点 FD @ t_γ=0.98γ_nat | **γ-hold FD 残差 < 0.02 eV/Å**（对照 proxy 的 0.502）——Ô_w 成立的判决点 | ⏳ 阻塞于 T-17（SCF 不收敛则 λ* 出不了收敛窗） |

**顺序**：T-11 →（过）→ T-12 → T-13 → T-14 →（T-15/T-16 顺手并入 T-12 的 commit）→ T-17 → T-18。
**进展（2026-08-12 晚）**：T-11~T-16 全部落地（R1–R6/R8，见
`2026-08-12-deltap-ow-t6mode.md`）；T-17 首测 FAIL（极限环定位完成，
非分支跳变）；本轮 commit 后 T-6' 继续 = **H_ow SCF 稳定化**
（内循环冻 θ / 混合 / θ 更新限幅）→ λ=±0.01 收敛 → 重跑 R5 全窗 → T-18。
**T-18 是 Ô_w 的判决点**：通过 → EFC 权重通道成立，评估 Ô_θ（L3.2）；
不通过 → 残差分解闭合计算后回来评审，不盲目转 Ô_θ。
