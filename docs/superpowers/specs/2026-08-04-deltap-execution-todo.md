...
| F-7 | L1 三件（PW Γ 记账 / ⟨η⟩ / spread_I 输出） | 各 ~0.5 天，可穿插 |
| F-8 | L2 应力（H_HK 应力 → V-H7；PW 应力） | RouteA++ 文档 §4 推导现成；**验收加"应力-极化 Maxwell 一致性"判据（评审 2026-08-13）**：∂σ/∂λ vs ∂P/∂ε 对照（F-2b 力版失配的应变版——H_HK 能量的应变依赖同走极化+几何双通道，V-H7 纯内部 FD 自洽不足以证明场物理）。做成与 EFC 对拍的前哨实验 |
